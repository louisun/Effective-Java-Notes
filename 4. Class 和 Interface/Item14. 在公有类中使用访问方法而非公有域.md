...
由于这些类的数据属性可以直接被访问，因此这些类不提供封装。 如果不更改API，则无法更改其表示形式，无法强制执行不变量，并且在访问属性时无法执行辅助操作。 坚持面向对象的程序员觉得这样的类很讨厌的，应该被具有私有属性和公共访问方法的类（`getter`）所取代，而对于可变类来说，它们应该被替换为`setter`设值方法：
