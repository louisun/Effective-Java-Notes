...
有时候，你可能会试图写一些**退化的类**，**除了集中实例属性之外别无用处**：
...
由于这些类的数据属性可以直接被访问，因此这些类不提供封装。 如果不更改API，则无法更改其表示形式，无法强制执行不变量，并且在访问属性时无法执行辅助操作。 坚持面向对象的程序员觉得这样的类很讨厌的，应该被具有私有属性和公共访问方法的类（`getter`）所取代，而对于可变类来说，它们应该被替换为`setter`设值方法：
...
Java平台类库中的几个类违反了公共类不应直接暴露属性的建议。 着名的例子包括`java.awt`包中的`Point`和`Dimension`类。 这些类别应该被视为警示性的示例，而不是模仿的例子。暴露`Dimension`的内部结构的决定是一个严重的性能问题，这个问题在今天仍然存在。
